...
function initReveal() {
  ...
  const revealProject = (el, delay = 0) => {
    window.setTimeout(() => {
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      ...
      const projects = visible.filter((entry) => entry.target.classList.contains("project-card"));
      const rest = visible.filter((entry) => !entry.target.classList.contains("project-card"));

      projects.forEach((entry, index) => {
        revealProject(entry.target, index * 130);
      });
